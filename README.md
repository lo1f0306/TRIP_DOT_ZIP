...
👉 본 프로젝트는
**LLM 기반 Agent 시스템**을 활용하여 **대화형 여행 계획 자동화**를 목표로 한다
...
A[User Input] --> B[Middleware<br/>욕설 필터링 / 요약 /<br/>개인정보 마스킹]
...
#### 1) 구조화된 쿼리 생성 (Self-Querying Retriever)

* 사용자의 자연어 질의를 그대로 검색하지 않고, <strong>검색어(query)</strong>와 <strong>필터 조건(filter)</strong>으로 분리하여 처리
* 예를 들어, `"경주 조용한 카페"`라는 요청은  
  → 검색어: `"조용한 카페"`  
  → 필터: `{"city": "경주"}`  
  와 같이 구조화된 형태로 변환

👉 이를 통해 단순 키워드 포함 여부가 아니라,  
**해당 지역 안에서만 사용자의 의도와 관련된 장소를 탐색**할 수 있다.

---

#### 2) Semantic Search 기반 1차 후보군 추출

* Place API로 수집한 장소 설명 및 리뷰 데이터를 Chroma DB에 저장
* 사용자 질의와 장소 리뷰/설명 임베딩 간의 유사도를 비교하여  
  **의미 기반으로 Top-K 후보 장소를 검색**
* 사용자의 페르소나나 동행 정보(예: 아이와 함께, 부모님과 함께)와 관련된 맥락이  
  리뷰에 포함되어 있을 경우, 단순 장소명 매칭보다 높은 관련성을 가진 후보를 추출 가능

👉 단순 키워드 검색보다  
**사용자 상황과 맥락을 반영한 1차 후보군 확보에 유리하다.**

---

#### 3) 중복 제거 기반 후보 정제

* 동일 장소에 대해 여러 리뷰가 검색 결과에 함께 포함될 수 있으므로,  
  `seen_place_ids` 기반 중복 제거 로직 적용
* 장소 단위로 유니크한 후보만 남겨  
  **후속 rerank 및 필터링 단계의 품질을 안정화**

👉 검색 결과의 중복을 줄여  
**더 다양한 장소 후보를 효율적으로 확보**할 수 있다.
...
E --> F[SelfQueryRetriever 기반<br/>유사도 검색]
